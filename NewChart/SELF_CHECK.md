...
------------------------------------------------------------------------
 ...
 v  S1    PASS  option глобально (RETRO 14), строка 487/492
 ...
 v  T6    PASS  hover не вызывает полный render()
 v  T5    PASS  скрытие тултипа снимается при показе
 v  S8    PASS  скрытие через style
 v  S10   PASS  есть универсальный парсер даты
 v  S11a  PASS  нет backticks / template-literals (RETRO 13)
 v  S11b  PASS  нет стрелочные функции (RETRO 13)
 v  S11c  PASS  нет let / const (RETRO 13)
...
------------------------------------------------------------------------
 ...
 v  E2   PASS  overlay смонтирован в хост, разметка не пустая (6178 символов)
 ...
 v  E12  PASS  hover не пересобирает разметку
 ...
 v  EV3  PASS  все виды триггеров макета есть в чарте
 v  EV4  PASS  цветных элементов не меньше, чем в макете (11 vs 11)
 v  EV5  PASS  палитра макета перенесена полностью
 !  EV6  WARN  нет в чарте (2): «Каталог продуктов», «Каталог продуктов — # (только здесь #)». Строки из данных здесь — норма, подписи интерфейса — потеря
 !  EV7  WARN  есть в чарте, но нет в макете (2): «Функциональная структура», «Функциональная структура — # (только зде». Проверь, не выдуманный ли это текст (RETRO 12)
------------------------------------------------------------------------
Итог: 25/27 PASS, 2 WARN, 0 N/A, 0 FAIL
```
...
| B3 | PASS | ET1-ET4 + E6: тултип виден на всех 6 триггерах, гаснет, пиксели меняются (RETRO 44); E12 hover не пересобирает разметку (RETRO 20) |
...
| B6 | WARN | сверка с макетом (`--vs`): EV6/EV7 — «Каталог продуктов» макета заменён на «Функциональная структура». Расхождение реальное, см. §4 NOTES |
