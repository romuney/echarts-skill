...
def strip_nums(txt, n):
    """Снять с правого края до n числовых токенов: «Грейд 19 18 1,7%» → «Грейд 19»."""
    s = re.sub(r'\s*[·|].*$', '', txt).strip()
    for _ in range(n):
        m = re.search(r'\s(\S+)$', s)
        if not m or not NUM_TOKEN.match(m.group(1)):
            break
        s = s[:m.start()].strip()
    return s


def categories(texts):
    """Подписи категорий из строк копий — В ПОРЯДКЕ МАКЕТА, или None.

    Сколько чисел справа лишние, заранее неизвестно: у «Банк 773 70,9%» их два,
    у «Штат 1 087 99,7%» — три, а у «Грейд 19 18 1,7%» число 19 это ЧАСТЬ
    подписи. Жадная чистка превращала все грейды в одинаковое «Грейд».
    Поэтому режем не по образцу строки, а по глобальному условию: снимаем
    столько, сколько можно, пока подписи ОСТАЮТСЯ РАЗНЫМИ. Различимость
    категорий — это то, ради чего список и нужен.
    """
    best = None
    for k in (2, 1, 0):
        cand = [strip_nums(t, k) for t in texts]
        if all(cand) and len(set(cand)) == len(cand):
            best = cand
            break
    # Дальше по одному числу не режем СПЕЦИАЛЬНО. «Штат 1 087» осталось бы
    # «Штат 1» — это видно глазами и правится за секунду. А «Грейд 19»,
    # срезанное до «Грейд», сливает десять категорий в одну молча, и всплывает
    # это уже нулями в группировке. Из двух ошибок черновика заметная лучше.
    return best


def order_draft(text, zones):
    """Черновик CFG.order: категории В ПОРЯДКЕ МАКЕТА, готовые к вставке.

    Порядок категорий — часть ТЗ (правило 14), и он нужен ДВАЖДЫ. По нему
    сортирует buildModel — иначе бары встанут по величине и «более 5 лет»
    уедет ниже «менее 1 года» (RETRO 54). И по нему же smoke.mjs строит
    автомок: без объявленных значений мок подставит 'k1' и число, ни одна
    группа в buildModel не совпадёт, половина виджета выйдет нулевой,
    а проверки тултипов и SVG уедут в WARN и N/A на пустом месте (RETRO 66).

    Переписывать 24 подписи руками из макета — это и потерянный контекст,
    и опечатки в значениях, по которым потом не сходится группировка.
    """
    out = []
    for z in zones:
        texts, _ = copy_texts(text, z)
        cats = categories(texts)
        if not cats or len(cats) < 2:
            continue          # подписи не различаются — это не список категорий
        if any(len(c) > 40 for c in cats):
            continue          # в копии не подпись, а целая карточка
        # Категория — это подпись, а не абзац. Копия из пяти слов с цифрами
        # внутри («Мужчины 686 Ср. возраст 32 г.») это карточка: в CFG.order
        # ей делать нечего, и черновик с таким мусором хуже пустого.
        if sum(1 for c in cats if len(c.split()) > 4) * 3 > len(cats):
            continue
        out.append((z, cats))
    return out
...
def main():
    ...
    if not only_plan:
        ...
        if zones:
            ...
            drafts = order_draft(text, zones)
            if drafts:
                print('')
                print('CFG.order — ЧЕРНОВИК ИЗ МАКЕТА, вставить в БЛОК 1')
                print('Ключ = имя колонки SQL, подставь сам. Если в одном повторе')
                print('лежат РАЗНЫЕ измерения (вкладки), разрежь список по смыслу.')
                print('Числа справа сняты эвристикой — сверь подписи глазами там,')
                print('где число есть в самой подписи («Штат 1 087» → «Штат 1»).')
                print('')
                print('  order: {')
                for z, cats in drafts:
                    vals = ', '.join("'" + c.replace("'", "\\'") + "'" for c in cats)
                    print("    '<колонка>': [" + vals + '],')
                    print('    //          ^ ' + str(z['n']) + '× ' + z['sig']
                          + ', строки ' + str(z['lo']) + '-' + str(z['hi2']))
                print('  }')
                print('')
                print('Он нужен ДВАЖДЫ: по нему сортирует buildModel (без него бары')
                print('встанут по величине и порядок макета потеряется, RETRO 54)')
                print('и по нему же smoke.mjs строит автомок — иначе мок не совпадёт')
                print('с разметкой ни одной группой, виджет выйдет нулевым, а проверки')
                print('тултипов и SVG уедут в WARN/N-A на пустом месте (RETRO 66).')
