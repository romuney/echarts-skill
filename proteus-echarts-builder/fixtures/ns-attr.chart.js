...
//
// ДВА ТИХИХ БАГА, которые доезжают до пользователя (RETRO 59, 61):
//   1. имя атрибута склеено с префиксом: в DOM приедет pvt-data-tip,
//      и весь тултиповый слой уйдёт в N/A вместо FAIL (T7b);
//   2. часть макета выброшена, решение оформлено комментарием (H4).
...
function buildHTML() {
  ...
  for (i = 0; i < VIEWS.length; i++) {
    h.push('<div class="' + P + '-view' + (VIEWS[i] === state.view ? ' active' : '')
      + '" data-view="' + VIEWS[i] + '">');
    // Группировка не реализована для простоты — строки подряд
    h.push('<span ' + CFG.ns + '-data-tip="row:' + VIEWS[i] + '">Экран ' + VIEWS[i].toUpperCase()
...
(function mount() {
  try {
    ...
    function getTip(el) { return el.getAttribute(CFG.ns + '-data-tip'); }
    ...
    overlay.addEventListener('mouseover', function (e) {
      var t = e.target.closest ? e.target.closest('[' + CFG.ns + '-data-tip]') : null;
      if (t) { renderTip(t); }
    });
