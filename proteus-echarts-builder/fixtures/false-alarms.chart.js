// ФИКСТУРА для `python3 validate.py --selftest`. В Proteus не вставляется.
//
// ИСПРАВНЫЙ ВИДЖЕТ, на котором валидатор РАНЬШЕ давал ложные FAIL (RETRO 60):
//   1. в buildCSS() рядом стоят '...' и "..." — S5 склеивала два литерала
//      и выдавала код между ними за «голые селекторы»;
//   2. в обработчике есть СВОЙ try/catch, и он стоит в файле раньше
//      монтажного — C6b судила по первому catch и роняла сдачу.
// Агент на такие FAIL идёт чинить чарт: переставляет кавычки и удаляет
// защитный catch. Ни одного FAIL здесь быть не должно.
...
function buildCSS() {
  ...
  return '<style>'
    ...
    + P + '-acts{display:flex;gap:' + pad + "px" + ';margin-bottom:6px;}'
    + P + '-note{padding:' + pad + "px" + ';color:#666;}'
